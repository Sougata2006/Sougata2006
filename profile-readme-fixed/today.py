...
USER_NAME = os.environ['USER_NAME']  # 'Sougata2006'
...
# Make sure the cache directory exists even on a fresh checkout.
# (Without this, cache_builder()'s open() call fails with FileNotFoundError
#  if the folder itself was never committed to the repo.)
os.makedirs('cache', exist_ok=True)


def account_age(created_at_iso):
    """
    Returns how long ago the GitHub account was created,
    e.g. '1 year, 11 months, 20 days'
    """
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    ...
    Returns the total number of lines of code across all repos.
    """
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    """
    Checks each repo to see if it's been updated since last cached; if so,
    reruns recursive_loc on it to refresh the LOC count.
    """
    cached = True
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, follower_data, following_data, loc_data):
    """
    Parse SVG files and update elements with account age, commits, stars,
    repos, followers, following, and lines of code.
    """
    ...
    justify_format(root, 'following_data', following_data, 4)
...
if __name__ == '__main__':
    """
    Sougata Paul (Sougata2006)
    Adapted from Andrew Grant's (Andrew6rant) original today.py
    """
    ...
    svg_overwrite('dark_mode.svg', age_data, commit_data, star_data, repo_data, follower_data, following_data, total_loc[:-1])
    svg_overwrite('light_mode.svg', age_data, commit_data, star_data, repo_data, follower_data, following_data, total_loc[:-1])
